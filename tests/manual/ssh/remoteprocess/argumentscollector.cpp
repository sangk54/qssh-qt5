    ...
    try {
        ...
        for (pos = 1; pos < m_arguments.count() - 1; ++pos)
        {
            if (checkAndSetStringArg(pos, host, "-h")
                || checkAndSetStringArg(pos, userName, "-u"))
                continue;
            ...
            if (checkAndSetStringArg(pos, password, "-pwd"))
            {
        ...
        else parameters.setPassword(password);

        if (userName.isEmpty())
        {
            userName = QProcessEnvironment::systemEnvironment().value(QLatin1String("USER"));
        }
